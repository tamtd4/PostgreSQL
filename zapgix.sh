...
usage() {
    ...
    echo "  -d            Specify databases to connect"
...
#################################################################################
while getopts "d:s::a:sj:u:p:hvt:" OPTION; do
    case ${OPTION} in
        d)
            db=${OPTARG}
            ;;
	...
	u)
	    auth_user=${OPTARG}
            #echo auth_user: "$OPTARG"
	    ;;
	p)
	    auth_pass=${OPTARG}
            #echo auth_pass: $OPTARG
	    ;;
	U)
	    UNIXUSER=${OPTARG}
            #echo $UNIXUSER
...
if [[ -f "${SQL%.sql}.sql" ]]; then
    if [[ -g ${auth_user} ]]; then
       ...
       cmd="psql -qAtX -d $db -U ${auth_user:-postgres} -w -f ${SQL%.sql}.sql"  
...
#if [[ -f "${SQL%.sql}.sql" ]]; then
#    #cmd="psql -qAtX -U ${auth_user:-postgres} -f ${SQL%.sql}.sql"
#    cmd="psql -qAtX -d mydb -U ${auth_user:-postgres} -w -f ${SQL%.sql}.sql"
#    #rval=`sudo su - ${UNIXUSER:-postgres} -c "${cmd} ${ARGS} 2>/dev/null"`
#    rval=`${cmd} ${ARGS} 2>/dev/null`
#    rcode="${?}"
#    if [[ ${rcode} == 0 && ${TIMING} =~ ^(on|ON|1|true|TRUE)$ ]]; then
